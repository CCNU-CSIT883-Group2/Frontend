<template>
  <div class="bg-surface-50 dark:bg-surface-950 w-screen h-screen flex items-center justify-center">
    <div class="bg-surface-0 dark:bg-surface-900 shadow-lg rounded-lg w-full max-w-6xl flex flex-col items-center p-6">
      <!-- Main Content -->
      <div class="flex-1 md:mr-10">
        <div class="bg-surface-0 dark:bg-surface-950 p-2 w-[800px] rounded-lg">
          ...
          <div class="text-surface-500 dark:text-surface-300 mb-10">Manage your profile, track progress, and explore more features.</div>
          ...
          <div class="bg-surface-100 dark:bg-surface-800 p-8 rounded-lg mb-6">
            <li class="flex items-center py-5 px-3 border-t border-surface flex-wrap">
              <div class="text-surface-500 dark:text-surface-300 w-6/12 md:w-2/12 font-medium text-lg">Role</div>
            ...
            <!-- Name -->
            <li class="flex items-center py-5 px-3 border-t border-surface flex-wrap">
              <div class="text-surface-500 dark:text-surface-300 w-6/12 md:w-2/12 font-medium text-lg">Name</div>
              <div v-if="!editingName" class="text-surface-900 dark:text-surface-0 w-full md:w-8/12">
                {{ newName }}
              </div>
              <div v-else class="w-full md:w-8/12">
                <input v-model="newName" type="text" class="input-field w-full h-10 text-lg" />
              </div>
            ...
            <!-- Email -->
            <li class="flex items-center py-5 px-3 border-t border-surface flex-wrap">
              <div class="text-surface-500 dark:text-surface-300 w-6/12 md:w-2/12 font-medium text-lg">Email</div>
              <div v-if="!editingEmail" class="text-surface-900 dark:text-surface-0 w-full md:w-8/12">
                {{ newEmail }}
              </div>
              <div v-else class="w-full md:w-8/12">
                <input v-model="newEmail" type="email" class="input-field w-full h-10 text-lg" />
              </div>
              <div class="w-6/12 md:w-2/12 flex justify-end">
            ...
            <!-- New Password -->
            <li class="flex items-center py-5 px-3 border-t border-surface flex-wrap">
              <div class="text-surface-500 dark:text-surface-300 w-6/12 md:w-2/12 font-medium text-lg">New Password</div>
              <div class="w-full md:w-8/12">
                <input v-model="newPassword" type="password" class="input-field mb-3 h-10 w-full text-lg" placeholder="New Password" @input="checkPasswordStrength" />
              </div>
            ...
            <li class="flex items-center py-5 px-3 border-t border-surface flex-wrap">
              <div class="text-surface-500 dark:text-surface-300 w-6/12 md:w-2/12 font-medium text-lg">Confirm Password</div>
              <div class="w-full md:w-8/12">
                <input v-model="confirmPassword" type="password" class="input-field mb-3 w-full h-10 text-lg" placeholder="Confirm Password" />
              </div>
              <div v-if="confirmPassword.length > 0" class="mt-1 w-full md:w-8/12">
                <!-- Password Match Status -->
                <span v-if="newPassword === confirmPassword" class="text-green-600 text-sm flex items-center justify-center">✔️ Passwords match</span>
                <span v-else class="text-red-600 text-sm flex items-center justify-center">Passwords do not match</span>
              </div>
            </li>
          ...
          <div class="flex justify-center mt-4 space-x-4">
            <Button label="Save Changes" icon="pi pi-check" text :disabled="!canSavePassword" @click="saveAllChanges" />
            <Button label="Log Out" icon="pi pi-sign-out" text class="p-button-danger" @click="logout" />
          </div>
        </div>
      </div>
    </div>
...
const canSavePassword = computed(() => newPassword.value && confirmPassword.value && passwordsMatch.value);
</script>
