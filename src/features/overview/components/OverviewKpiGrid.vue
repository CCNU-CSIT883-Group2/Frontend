<template>
  <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
    ...
    >
      <div class="inline-flex h-8 w-8 items-center justify-center rounded-lg" :class="iconToneClass(card.tone)">
        <i :class="card.icon" class="text-sm" />
      </div>
      <p class="mt-3 text-xs font-medium uppercase tracking-wide text-surface-500 dark:text-surface-300">
        {{ card.label }}
      </p>
      <p class="mt-2 text-2xl font-semibold text-surface-900 dark:text-surface-0">{{ card.value }}</p>
